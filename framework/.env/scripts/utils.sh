...
installChaincodePKG(){
    ...
    peer chaincode install "$PKG_PATH" >&log.txt
    res=$?
    set +x
    cat log.txt
    verifyResult $res "Chaincode installation on peer${PEER}.org${ORG} has failed"
...
# TODO common module factoring
instantiateChaincodeWithCollection() {
  PEER=$1
  ORG=$2
  setGlobals $PEER $ORG
  VERSION=${3:-1.0}
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n $cc_name \
        -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["init"]}' \
        -P "AND ('Org1MSP.peer','Org2MSP.peer')" \
        --collections-config $collection >&log.txt
    res=$?
    set +x
  else
    set -x
...
chaincodeInvokeWithNameOfPDC() {
  # TODO parsePeerConnectionParameter
  #parsePeerConnectionParameters $@
  #res=$?
  #verifyResult $res "Invoke transaction failed on channel '$CHANNEL_NAME' due to uneven number of peer and org parameters "
  PEER_ADDRESS="--peerAddresses peer0.org1.example.com:7051 --tlsRootCertFiles $PEER0_ORG1_CA --peerAddresses peer0.org2.example.com:9051 --tlsRootCertFiles $PEER0_ORG2_CA"
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode invoke -o orderer.example.com:7050 -C $CHANNEL_NAME $PEER_ADDRESS \
        -n $cc_name -c $args --transient "$transient_data" >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode invoke -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME \
        $PEER_ADDRESS -n $cc_name -c $args --transient "$transient_data" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
# TODO : compare with mutli peer
chaincodeQueryWithName() {
  PEER=$1
  ORG=$2
  cc_name=$3
  ch_name=$4
  ...
  if [ "x$transient_data" == "x" ]; then 
      set -x
      res=$(peer chaincode query -C $ch_name -n $cc_name -c $args)
      set +x
      echo $res
  else
      set -x
      res=$(peer chaincode query -C $ch_name -n $cc_name -c $args --transient "$transient_data")
      set +x
      echo $res
  fi
}
